...
- **Next.js 15.1.6** (App Router) ✅ **移行完了**
- **React 18.2.0**
- **TypeScript 5.3.3** (strictモード有効)
...
├── pages/             # レガシー残存（_app.tsx, _document.tsx, 404/500, api/, places-and-groups.tsx）
├── _pages/            # 旧版退避（空）
├── _backup/           # バックアップ（pages-router/）
├── components/        # 再利用可能なUIコンポーネント
│   ├── FormattedDate.tsx    # ✅ 統合済み
│   ├── DropdownMenu.tsx     # ✅ 汎用ドロップダウン（Navbarで使用）
│   ├── AppRouter*.tsx       # App Router用コンポーネント（layout.tsxから参照）
│   └── [legacy].tsx         # レガシー（Header/Footer/Navbar/SiteLogo/Link/NavbarCollapseButton 等は実質未使用）
├── lib/               # ユーティリティ関数とシステム
│   ├── constants.ts   # ✅ アプリケーション定数
│   ├── env.ts         # ✅ 環境変数管理
│   ├── microcms-app-router.ts # microCMS クライアント（App Router用、filters対応）
...
- `DropdownMenu` - 汎用ドロップダウン（Navbar の活動報告メニュー等）

**統合予定**: `layout.tsx` は `AppRouter*` 系を使用中で、素の以下コンポーネントは実質未使用。
リネーム（AppRouter プレフィックス除去）または旧版削除を検討中：

- `SiteLogo` vs `AppRouterSiteLogo`
- `NavbarCollapseButton` vs `AppRouterNavbarCollapseButton`
- `Link` vs `AppRouterLink`
- `Header` / `Footer` / `Navbar` （素のものは未参照）
...
**App Router**: Server Components + Next.js 15 キャッシュ機能（`revalidate` / `tags`）。`newsApi.getAll({ filters })` で「お知らせ / 活動報告」を API 側でフィルタリング可能。
...
- **完了**: 「お知らせ」と「活動報告」の表示分離（`NEWS_CONFIG.REPORT_STATUS = 'レポート'` で判別）
- **完了**: microCMS クライアントに `filters` 機能を追加し、API 側でフィルタリング
- **完了**: 活動報告メニューをドロップダウン化、汎用 `DropdownMenu` コンポーネントを導入
- **完了**: フッター・News ボタンの文言を「活動報告」に統一（2026-01）

#### ✅ その他の完了済み

- **完了**: OG画像の動的生成（`app/api/og/`、`layout.tsx` の `openGraph.images` で参照）
- **完了**: フッターの SNS リンクに Instagram 追加
- **完了**: 「支援」→「サポート」への文言変更
...
`layout.tsx` は `AppRouter*` 系を使用中。素のレガシー版は実質未参照のため、リネーム or 削除のいずれかで整理：

1. **SiteLogo** - 統合が容易
2. **NavbarCollapseButton** - アクセシビリティ改善版を採用
3. **Link** - 外部リンク機能の統合が必要
4. **Header / Footer / Navbar** - 素の方は未参照、削除候補

#### 🔄 残タスク

- [ ] `pages/places-and-groups.tsx` の App Router 移行
- [ ] `_pages/`（空）・`_backup/` の整理
- [ ] ダークモード実装の完了
- [ ] パフォーマンス最適化
- [ ] テストの追加
- [ ] アクセシビリティの向上
...
- **キャッシュ**: Next.js 15 の `revalidate` / `tags` を活用
...
**最終更新**: 2026-05-18 - 「お知らせ / 活動報告」分離、汎用 DropdownMenu 導入、OG画像実装などの現状を反映
